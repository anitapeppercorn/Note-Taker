...
function allButId(id, notesArray) {
   const result = notesArray.filter(note => note.id !== id);// return a list of notes that do NOT match the ID
   return result;
};
...
//This function updates the JSON file in the db (database) folder
...
//This function validates that the data content entered from the client terminal meets 
...
//we export these functions. they are used in the api routes files
module.exports = {
   findById,
   allButId,
   overWritenote,
