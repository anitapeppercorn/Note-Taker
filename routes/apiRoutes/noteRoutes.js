...
// This is the GET route to get data from notes.json
...
// This is the POST route to update data from client to notes.json
router.post('/notes', (req, res) => { 
   if (!validatenote(req.body)) {
       res.status(400).send('The note to POST is not properly formatted.');
   } else {
    req.body.id = (0 + ((new Date()).getTime()))+'';//We make a new unique ID and convert it to a string
    //req.body.id = body.length.toString();// we do not use this as the length of our array keeps changing if we delete notes
...
// This is the DELETE route to update notes.json after removing the deleted note identified by ID
router.delete('/notes/:id', (req, res) => {
    ...
    if (req.params.id) {
        ...
        result.remaining_notes = newNotes; //beware newNotes can be a very long array
